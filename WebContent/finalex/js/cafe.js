$(document).ready(function() {
	...
	// 인기글 가져오기
	$.ajax({
		url:"./data/poparticle.xml",
		type:"get",
		//data:"",
		success:function(data){
			//console.log(data)
			$("#tbl_poparticle").empty();
			let $article = $(data).find("article");
			console.log($article);
			$($article).each(function(idx, item){
				console.log(item)
				let $item = $(item);
				let $tr = $("<tr><td>"+$item.find("title").text()+"</td><td>"+$item.find("writer").text()+"</td><td>"+$item.find("hitcnt").text()+"</td></tr>");
				$("#tbl_poparticle").append($tr);
			})
		}
	})
	...
	// 최신 글 가져오기
	$.ajax({
		url:"./data/newarticle.json",
		type:"get",
		//data:"",
		success:function(data){
			$("#tbl_newarticle").empty();
			let $article = $(data).find("article");
			$(data).each(function(idx, item){
				let $tr = $("<tr><td>"+item.title+"</td><td>"+item.writer+"</td><td>"+item.hitcnt+"</td></tr>");
				$("#tbl_newarticle").append($tr);
				console.log(item)
			})
		}
	})
...
/*
	...
	*/
	...
	setInterval(getPopularMenu, 5000);
